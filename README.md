...
4. **Menambah data sampah** — Mengunggah foto sampah + mengisi berat, lalu menekan **"Hitung"** untuk mengklasifikasikan sampah (kategori, harga per kg, nominal koin) kemudian **"Submit"** (mengirim transaksi ke backend).
...
│   │   ├── transaksiService.js # Endpoint transaksi (list, per user, detail, gambar, submit)
...
```bash
git clone https://github.com/brainhacker21/pilah-pinter-fe.git
cd pilah-pinter-fe
```
...
- Link ke halaman registrasi: _"Belum daftar akun? Daftar sekarang"_
...
- Link ke halaman login: _"Sudah punya akun? Login sekarang"_
...
- Teks: _"Dapatkan koin untuk tentukan jenis sampah"_
- Sub-teks: _"Siapkan file gambar atau kamera anda"_
...
- Ikon upload besar + pesan _"Daftar sampah anda kosong"_

**Loading State**

- Saat fetch berjalan, menampilkan _"Memuat data..."_
...
Halaman untuk menambahkan data sampah baru dengan alur **dua tahap**: **Hitung** (klasifikasi via API) lalu **Submit** (konfirmasi + kirim transaksi ke backend). Terdiri dari area upload gambar dan form berat.
...
- **Dinonaktifkan** (opacity redup, `cursor-not-allowed`) sampai field **Berat** diisi angka positif yang valid. Sebelum itu tampil pesan _"Isi berat terlebih dahulu untuk mengunggah gambar"_
- Setelah berat valid: bisa klik / drag file → tampilkan preview full-cover + teks _"Klik untuk ganti gambar"_
...
- Judul _"Konfirmasi"_, body konfirmasi data, tombol "Batal" dan "Kirim"
...
| `error`        | `object`         | Error state (berat & general)                           |
...
- **Gambar** wajib diupload sebelum menghitung (alert _"Silakan upload gambar terlebih dahulu."_)
...
6. Klik **"Kirim"** → panggil `postSubmitTransaksi(...)` → dialog tertutup → setelah 1500ms redirect otomatis ke `/home`
...
- Teks: _"Detail transaksi sampah"_ + sub-teks _"Rincian sampah yang telah kamu setor"_
...
- Fallback ikon + teks _"Gambar tidak tersedia"_ bila gambar gagal dimuat
...
- Saat fetch berjalan menampilkan _"Memuat data..."_
- Jika gagal / data tidak ada, menampilkan error state
...
- **Request interceptor** — menyisipkan header `Authorization: Bearer ` dari `localStorage` pada setiap request.
...
| `transaksiService.js` | `getAllTransaksi`, `getTransaksiByUser`, `getTransaksiById`, `getTransaksiGambar`, **`postSubmitTransaksi`** | `/transaksi`, `/transaksi/user/:userId`, `/transaksi/:id`, `/transaksi/:id/gambar`, `POST /transaksi/submit` |
...
5. **Di TambahSampahPage:**
   ...
   - Dialog konfirmasi → "Kirim" → `postSubmitTransaksi(...)` → kembali ke `/home`
...
- **Backend terintegrasi** — Autentikasi, daftar transaksi, klasifikasi sampah, dan submit transaksi memakai REST API asli via Axios. Token JWT disimpan di `localStorage` dengan auto-refresh saat `401`. Base URL diatur lewat `VITE_API_URL` (`.env`), dan `vite.config.js` menyediakan proxy `/api` untuk dev lokal.
